...
func (k Keeper) GetHistoricalInfo(ctx context.Context, height int64) (stakingtypes.HistoricalInfo, error) {
	histInfoSDK := stakingtypes.HistoricalInfo{}

	histInfo, err := k.GetZenrockHistoricalInfo(ctx, height)
	if err != nil {
		return histInfoSDK, err
	}

	histInfoSDK.Valset = make([]stakingtypes.Validator, len(histInfo.Valset))
	for i, val := range histInfo.Valset {
		histInfoSDK.Valset[i] = zenrockValidatorToSDKValidator(val)
	}

	histInfoSDK.Header = histInfo.Header

	return histInfoSDK, nil
}

// GetHistoricalInfo gets the historical info at a given height
func (k Keeper) GetZenrockHistoricalInfo(ctx context.Context, height int64) (types.HistoricalInfoHV, error) {
	store := k.storeService.OpenKVStore(ctx)
	key := types.GetHistoricalInfoKey(height)

	value, err := store.Get(key)
	if err != nil {
		return types.HistoricalInfoHV{}, err
	}

	if value == nil {
		return types.HistoricalInfoHV{}, types.ErrNoHistoricalInfo
	}
...
func (k Keeper) SetHistoricalInfo(ctx context.Context, height int64, hi *types.HistoricalInfoHV) error {
	store := k.storeService.OpenKVStore(ctx)
	key := types.GetHistoricalInfoKey(height)
...
func (k Keeper) IterateHistoricalInfo(ctx context.Context, cb func(types.HistoricalInfoHV) bool) error {
	store := k.storeService.OpenKVStore(ctx)
...
func (k Keeper) GetAllHistoricalInfo(ctx context.Context) ([]types.HistoricalInfoHV, error) {
	var infos []types.HistoricalInfoHV
	err := k.IterateHistoricalInfo(ctx, func(histInfo types.HistoricalInfoHV) bool {
