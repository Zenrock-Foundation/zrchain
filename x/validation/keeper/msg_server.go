...
func (k msgServer) EditValidator(ctx context.Context, msg *types.MsgEditValidator) (*types.MsgEditValidatorResponse, error) {
	...
	if msg.CommissionRate != nil {
		...
		if msg.CommissionRate.LT(k.MinCommissionRate(ctx)) {
			return nil, errorsmod.Wrapf(sdkerrors.ErrInvalidRequest, "commission rate cannot be less than the min commission rate %s", k.MinCommissionRate(ctx).String())
		}
	}
	...
	if msg.MinSelfDelegation != nil {
		...
		if msg.MinSelfDelegation.GT(validator.TokensNative) {
...
// UpdateParams defines a method to perform updation of params exist in x/validation module.
