...
func (k Keeper) GetDelegation(ctx context.Context, delAddr sdk.AccAddress, valAddr sdk.ValAddress) (stakingtypes.Delegation, error) {
	delegation, err := k.GetZenrockDelegation(ctx, delAddr, valAddr)
	if err != nil {
		return stakingtypes.Delegation{}, err
	}
	return stakingtypes.Delegation(delegation), nil
}

// GetDelegation returns a specific delegation.
func (k Keeper) GetZenrockDelegation(ctx context.Context, delAddr sdk.AccAddress, valAddr sdk.ValAddress) (types.Delegation, error) {
...
func (k Keeper) IterateAllDelegations(ctx context.Context, cb func(delegation stakingtypes.Delegation) (stop bool)) error {
	store := k.storeService.OpenKVStore(ctx)
	...
	for ; iterator.Valid(); iterator.Next() {
		...
		if cb(stakingtypes.Delegation(delegation)) {
			break
		}
	}
...
func (k Keeper) GetAllDelegations(ctx context.Context) (delegations []stakingtypes.Delegation, err error) {
	err = k.IterateAllDelegations(ctx, func(delegation stakingtypes.Delegation) bool {
...
// delegator.
func (k Keeper) GetDelegatorDelegations(ctx context.Context, delegator sdk.AccAddress, maxRetrieve uint16) (delegations []stakingtypes.Delegation, err error) {
	...
	for ; iterator.Valid() && i < int(maxRetrieve); iterator.Next() {
		...
		delegations[i] = stakingtypes.Delegation(delegation)
		i++
	}
...
func (k Keeper) GetDelegatorBonded(ctx context.Context, delegator sdk.AccAddress) (math.Int, error) {
	...
	err := k.IterateDelegatorDelegations(ctx, delegator, func(delegation stakingtypes.Delegation) bool {
...
func (k Keeper) IterateDelegatorDelegations(ctx context.Context, delegator sdk.AccAddress, cb func(delegation stakingtypes.Delegation) (stop bool)) error {
	store := k.storeService.OpenKVStore(ctx)
...
func (k Keeper) Delegate(
	...
	validator types.ValidatorHV, subtractAccount bool,
	...
	delegation, err := k.GetZenrockDelegation(ctx, delAddr, valbz)
	if err == nil {
		// found
...
func (k Keeper) BeginRedelegation(
	...
	dstValidator, err := k.GetZenrockValidator(ctx, valDstAddr)
	...
	srcValidator, err := k.GetZenrockValidator(ctx, valSrcAddr)
	if errors.Is(err, types.ErrNoValidatorFound) {
