...
func (k Keeper) GetValidator(ctx context.Context, addr sdk.ValAddress) (stakingtypes.Validator, error) {
	v, err := k.GetZenrockValidator(ctx, addr)
	if err != nil {
		return stakingtypes.Validator{}, err
	}
	return zenrockValidatorToSDKValidator(v), nil
}

// GetValidator gets a single validator
func (k Keeper) GetZenrockValidator(ctx context.Context, addr sdk.ValAddress) (validator types.ValidatorHV, err error) {
...
func (k Keeper) GetZenrockValidatorFromBech32(ctx context.Context, addr string) (validator types.ValidatorHV, err error) {
	validatorAddr, err := sdk.ValAddressFromBech32(addr)
	if err != nil {
		return validator, err
	}
	return k.GetZenrockValidator(ctx, validatorAddr)
}

func (k Keeper) mustGetValidator(ctx context.Context, addr sdk.ValAddress) types.ValidatorHV {
	validator, err := k.GetZenrockValidator(ctx, addr)
	if err != nil {
...
func (k Keeper) GetValidatorByConsAddr(ctx context.Context, consAddr sdk.ConsAddress) (validator types.ValidatorHV, err error) {
	store := k.storeService.OpenKVStore(ctx)
	...
	return k.GetZenrockValidator(ctx, opAddr)
}

func (k Keeper) mustGetValidatorByConsAddr(ctx context.Context, consAddr sdk.ConsAddress) types.ValidatorHV {
...
func (k Keeper) SetValidator(ctx context.Context, validator types.ValidatorHV) error {
	store := k.storeService.OpenKVStore(ctx)
...
func (k Keeper) SetValidatorByConsAddr(ctx context.Context, validator types.ValidatorHV) error {
...
func (k Keeper) SetValidatorByPowerIndex(ctx context.Context, validator types.ValidatorHV) error {
...
func (k Keeper) DeleteValidatorByPowerIndex(ctx context.Context, validator types.ValidatorHV) error {
	store := k.storeService.OpenKVStore(ctx)
...
func (k Keeper) SetNewValidatorByPowerIndex(ctx context.Context, validator types.ValidatorHV) error {
	store := k.storeService.OpenKVStore(ctx)
	str, err := k.validatorAddressCodec.StringToBytes(validator.GetOperator())
	if err != nil {
		return err
...
func (k Keeper) AddValidatorTokensAndShares(ctx context.Context, validator types.ValidatorHV,
	tokensToAdd math.Int,
) (valOut types.ValidatorHV, addedShares math.LegacyDec, err error) {
...
func (k Keeper) RemoveValidatorTokensAndShares(ctx context.Context, validator types.ValidatorHV,
	sharesToRemove math.LegacyDec,
) (valOut types.ValidatorHV, removedTokens math.Int, err error) {
	err = k.DeleteValidatorByPowerIndex(ctx, validator)
	if err != nil {
...
func (k Keeper) RemoveValidatorTokens(ctx context.Context,
	validator types.ValidatorHV, tokensToRemove math.Int,
) (types.ValidatorHV, error) {
...
func (k Keeper) UpdateValidatorCommission(ctx context.Context,
	validator types.ValidatorHV, newRate math.LegacyDec,
...
func (k Keeper) GetAllValidators(ctx context.Context) (validators []stakingtypes.Validator, err error) {
	vals, err := k.GetAllZenrockValidators(ctx)
	for _, val := range vals {
		validators = append(validators, zenrockValidatorToSDKValidator(val))
	}
	return validators, err
}

// GetAllValidators gets the set of all validators with no limits, used during genesis dump
func (k Keeper) GetAllZenrockValidators(ctx context.Context) (validators []types.ValidatorHV, err error) {
	store := k.storeService.OpenKVStore(ctx)
...
func (k Keeper) GetValidators(ctx context.Context, maxRetrieve uint32) (validators []types.ValidatorHV, err error) {
	store := k.storeService.OpenKVStore(ctx)
	validators = make([]types.ValidatorHV, maxRetrieve)
...
func (k Keeper) GetBondedZenrockValidatorsByPower(ctx context.Context) ([]types.ValidatorHV, error) {
	maxValidators, err := k.MaxValidators(ctx)
	if err != nil {
		return nil, err
	}
	validators := make([]types.ValidatorHV, maxValidators)
...
func (k Keeper) GetLastValidators(ctx context.Context) (validators []types.ValidatorHV, err error) {
	store := k.storeService.OpenKVStore(ctx)
...
func (k Keeper) InsertUnbondingValidatorQueue(ctx context.Context, val types.ValidatorHV) error {
...
func (k Keeper) DeleteValidatorQueue(ctx context.Context, val types.ValidatorHV) error {
	addrs, err := k.GetUnbondingValidators(ctx, val.UnbondingTime, val.UnbondingHeight)
	if err != nil {
		return err
	}
	...
	for ; unbondingValIterator.Valid(); unbondingValIterator.Next() {
		...
		if keyHeight <= blockHeight && (keyTime.Before(blockTime) || keyTime.Equal(blockTime)) {
			...
			for _, valAddr := range addrs.Addresses {
				...
				val, err := k.GetZenrockValidator(ctx, addr)
				if err != nil {
...
// zenrockValidatorToSDKValidator converts a zenrock Validator to a vanilla x/staking SDK Validator.
// This omits the TokensAVS field, which represents the total AVS tokens delegated to the validator.
// The gov module uses the vanilla validator type, but this is not an issue as ROCK is the governance token.
func zenrockValidatorToSDKValidator(v types.ValidatorHV) stakingtypes.Validator {
	commission := stakingtypes.Commission{
		CommissionRates: stakingtypes.CommissionRates(v.Commission.CommissionRates),
	...
	return stakingtypes.Validator{
		...
		Tokens:                  v.TokensNative, // we don't use include TokensAVS here as only ROCK is used for governance
		DelegatorShares:         v.DelegatorShares,
		Description:             stakingtypes.Description(v.Description),
		UnbondingHeight:         v.UnbondingHeight,
		UnbondingTime:           v.UnbondingTime,
		Commission:              commission,
		MinSelfDelegation:       v.MinSelfDelegation,
		UnbondingOnHoldRefCount: v.UnbondingOnHoldRefCount,
		UnbondingIds:            v.UnbondingIds,
	}
}
