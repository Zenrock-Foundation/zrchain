...
func (k Keeper) Slash(ctx context.Context, consAddr sdk.ConsAddress, infractionHeight, power int64, slashFactor math.LegacyDec) (math.Int, error) {
	...
	tokensToBurn := math.MinInt(remainingSlashAmount, validator.TokensNative)
	...
	if tokensToBurn.IsZero() {
		...
		logger.Info(
			...
			"burned", tokensToBurn,
			"validatorTokens", validator.TokensNative,
		)
		return math.NewInt(0), nil
	}

	// we need to calculate the *effective* slash fraction for distribution
	if validator.TokensNative.IsPositive() {
		effectiveFraction := math.LegacyNewDecFromInt(tokensToBurn).QuoRoundUp(math.LegacyNewDecFromInt(validator.TokensNative))
	...
	slashAmountDecAVS := math.LegacyNewDecFromInt(validator.TokensAVS).Mul(slashFactor)
	slashAmountAVS := slashAmountDecAVS.TruncateInt()
	validator.TokensAVS = validator.TokensAVS.Sub(slashAmountAVS)

	if err := k.SetValidator(ctx, validator); err != nil {
		return math.NewInt(0), err
	}

	if _, err := k.CreateSlashEvent(
		sdkCtx,
		&types.SlashEvent{
			BlockHeight:         sdkCtx.BlockHeight(),
			ValidatorAddr:       validator.GetOperator(),
			PercentageSlashed:   slashFactor,
			TokensSlashedNative: tokensToBurn,
			TokensSlashedAVS:    slashAmountAVS,
		},
	); err != nil {
		return math.NewInt(0), err
	}
	...
	logger.Info(
		...
		"validator", validator.GetOperator(),
		"slash_factor", slashFactor.String(),
		"burned_native", tokensToBurn,
		"slashed_avs", slashAmountAVS,
	)
...
func (k Keeper) SlashWithInfractionReason(ctx context.Context, consAddr sdk.ConsAddress, infractionHeight, power int64, slashFactor math.LegacyDec, _ stakingtypes.Infraction) (math.Int, error) {
...
func (k Keeper) SlashRedelegation(ctx context.Context, srcValidator types.ValidatorHV, redelegation types.Redelegation,
	infractionHeight int64, slashFactor math.LegacyDec,
) (totalSlashAmount math.Int, err error) {
	sdkCtx := sdk.UnwrapSDKContext(ctx)
	now := sdkCtx.BlockHeader().Time
