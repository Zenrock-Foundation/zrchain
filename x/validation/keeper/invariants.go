...
func ModuleAccountInvariants(k *Keeper) sdk.Invariant {
	return func(ctx sdk.Context) (string, bool) {
		...
		err = k.IterateValidators(ctx, func(_ int64, validator stakingtypes.ValidatorI) bool {
...
func NonNegativePowerInvariant(k *Keeper) sdk.Invariant {
	return func(ctx sdk.Context) (string, bool) {
		...
		for ; iterator.Valid(); iterator.Next() {
			validator, err := k.GetZenrockValidator(ctx, iterator.Value())
			if err != nil {
			...
			if validator.TokensNative.IsNegative() || validator.TokensAVS.IsNegative() {
				broken = true
