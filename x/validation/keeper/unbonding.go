...
func (k Keeper) GetValidatorByUnbondingID(ctx context.Context, id uint64) (val types.ValidatorHV, err error) {
	store := k.storeService.OpenKVStore(ctx)

	valKey, err := store.Get(types.GetUnbondingIndexKey(id))
	if err != nil {
		return types.ValidatorHV{}, err
	}

	if valKey == nil {
		return types.ValidatorHV{}, types.ErrNoValidatorFound
	}
...
func (k Keeper) SetValidatorByUnbondingID(ctx context.Context, val types.ValidatorHV, id uint64) error {
	store := k.storeService.OpenKVStore(ctx)
