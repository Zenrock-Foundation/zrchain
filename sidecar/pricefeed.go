...
func (o *Oracle) initPriceFeed() (*ethclient.Client, *aggregatorv3.AggregatorV3Interface) {
	...
	priceFeedAddr := common.HexToAddress(o.Config.EthOracle.ContractAddrs.PriceFeed)
	// priceFeed, err := aggregatorv3.NewAggregatorV3Interface(priceFeedAddr, o.ethClient)
	priceFeed, err := aggregatorv3.NewAggregatorV3Interface(priceFeedAddr, tempEthClient) // use tempEthClient for now
	if err != nil {
		log.Fatalf("Failed to create Chainlink price feed instance: %v", err)
	}

	return tempEthClient, priceFeed
}

func (o *Oracle) fetchEthPrice(priceFeed *aggregatorv3.AggregatorV3Interface, blockNumber *big.Int) (float64, error) {
